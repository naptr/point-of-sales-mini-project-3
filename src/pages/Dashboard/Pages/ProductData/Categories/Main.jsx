  ...
  return (
    <div className="flex flex-col flex-grow justify-between relative">
      ...
      <div id="products-list-title" className="w-full h-10 flex justify-between items-center text-purple-500">
        ...
        <button id="add-products" className="h-full hover:bg-green-100 w-28 rounded flex items-center justify-center font-caption text-sm text-green-400 transition-all duration-300" onClick={handleNewCategory}>
          <p>New Categories</p>
        </button>
      </div>
      <div id="categories-container" className="min-h-custom w-full grid grid-cols-5 gap-4 place-content-start">
        {
          categories?.data.data.map(category => <Card item={category} key={category.id} identifier={category.id} />)
        }
      </div>
